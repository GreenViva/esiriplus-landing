      ...
      <div className="section-container">
        ...
        >
          {cards.map((card, i) => (
            <div
              key={card.titleKey}
              className={`card card-hover-border group text-center ${
                cardsVisible ? "animate-fade-in-up" : "scroll-hidden"
              }`}
              style={{ animationDelay: `${i * 150}ms` }}
            >
              <div className="w-14 h-14 bg-brand-teal-light text-brand-teal rounded-2xl flex items-center justify-center mx-auto mb-5 group-hover:bg-brand-teal group-hover:text-white transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 icon-hover-rotate">
                {card.icon}
              </div>
